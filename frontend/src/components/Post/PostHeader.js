...
const PostHeader = ({data}) => {
  ...
  const threadDelete = async (data) => {
    if (window.confirm("Are you sure you want to delete the thread\n" + data.title + "?")) {
      const res = await fetch(`${process.env.REACT_APP_SITE_URL}/api/thread/delete/` + data.id, {
        method: 'DELETE',
        headers: {
    ...
    <>
      ...
      <div className={'content-list'} style={{display: 'flex', gap: '15px'}}>
        ...
        <div style={{display: 'flex', flexDirection: 'column', justifyContent: 'space-between'}}>
          ...
          <Text>{data.text}</Text>
          {(data.author.signature && data.author.signature.length) &&
          <div><Divider dashed/><p>{data.author.signature}</p><Divider dashed/></div>}
          <Space size="large">
            <Space id="test" size={"large"}>
              {data.createdAt && <Text type='secondary'
                                       style={{fontSize: '11px'}}><ClockCircleOutlined/> {format(new Date(data.createdAt), 'dd. MMM yyyy')}
              </Text>}
            </Space>
            {
